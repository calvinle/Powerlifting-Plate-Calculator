...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _25kg.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
        ...
        collars.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView,boolean isChecked) {
                String rawWT = inputWeight.getText().toString();
                if (rawWT.matches("")){
    ...
    public void calculate(double wt){
        if (wt % 0.5 != 0){ return; }
        if (wt <= 20 && collars.isChecked() == false){ return; }
       
        double netWT = wt - 20;
        if (collars.isChecked()){ netWT -= 5; }
